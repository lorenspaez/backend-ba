...
export class AlertService {
  ...
  async getAlertById(alertId: number) {
    ...
    if (alert.id == null) {
      throw new ForbiddenException('No existe la alerta');
    }
  ...
  async editAlertByKey(alertKey: string, dto: UpdateAlertDto) {
    const alert = await this.prisma.alert.findFirst({
      where: {
        alertKey: alertKey,
      },
  ...
  async takeAlert(alertId: number, volunteerId: number, dto: TakeAlertDto) {
    ...
    if (volunteer.isVolunteer == false) {
      throw new ForbiddenException('Solo los voluntarios pueden tomar casos');
    }

    if (volunteer.takenAlertId != null) {
      throw new ForbiddenException('Ya tienes otro caso tomado');
    }

    if (volunteer.alertKey != null) {
      throw new ForbiddenException('Ya tienes un caso creado actualmente');
    }
    ...
    if (alert.status == 'Tomado') {
      throw new ForbiddenException('El caso ya está siendo atendido');
    }
    ...
    await this.httpService.post('https://fcm.googleapis.com/fcm/send', {
      data: {
        to: 'ept61Yr9QnGPh5GLkItyU3:APA91bFVAA_7j_ZMZ2Sv6Lk0QztRN7R69HiZphvab-7TPCegFLsFOTZ7PneQuzoMzzw8d2gtY3Kj5rYM3yrMjQae2CWO0-OCDIE6-Xi1D92KNWpKNcv0DH7Q0WGD9JXTBpggoM0LEQaT',
        notification: {
          body: 'wapo',
          title: 'jiji',
        },
  ...
  async cloneAlert(alertId: number, volunteerId: number, dto: CreateAlertDto) {
    const alert = await this.prisma.alert.findUnique({
      where: {
        id: alertId,
      },
  ...
  async leaveAlert(volunteerId: number, alertId: number) {
    await this.prisma.user.update({
      where: {
        id: volunteerId,
      },
  ...
  async closeAlert(alertId: number, volunteerId: number) {
    const volunteer = await this.prisma.user.findUnique({
      where: {
        id: volunteerId,
      },
    ...
    if (volunteer.isVolunteer == false) {
      throw new ForbiddenException('Solo los voluntarios pueden cerrar casos');
    }
  ...
  async deleteAlertByKey(alertKey: string) {
    const alert = await this.prisma.alert.findFirst({
      where: {
        alertKey: alertKey,
      },
